...
function () {
  ...
  function cskewT(Pascent, Tascent, Tdascent, startpressure, endpressure) {
    ...
    function skewT_main(Pascent, Tascent, Tdascent) {
      ...
      Tascent.forEach(function (t, index) {
        if (t < -300) Tascent[index] = null;
      });
      Tdascent.forEach(function (td, index) {
  ...
  function cbarbs(Pascent, Tascent, U, V, current_timestamp, dataOptions, cmaxP, cminP) {
    ...
    svg.append("g").append("text").html(model + '\xa0\xa0\xa0' + date).attr("x", w - 0.64 * w).attr("y", 0.035 * h).attr("font-family", "sans-serif").attr("font-family", "Arial").attr("font-size", 0.03 * h + "px").attr("fill", "#cccccc").attr("id", "statsID");
    svg.append("g").append("text").html(date).attr("x", w - 0.4 * w).attr("y", 0.085 * h).attr("font-family", "sans-serif").attr("fill", '#bdbdbd').attr("font-family", "Arial").attr("font-size", 0.03 * h + "px");
    svg.append("g").append("text").html(date).attr("x", w - 0.402 * w).attr("y", 0.082 * h).attr("font-family", "sans-serif").attr("fill", 'black').attr("font-family", "Arial").attr("font-size", 0.03 * h + "px").attr("id", "DateLabel");
  ...
  var SkewTApiPath = 'https://apiv2.skewt.org';
  var sondeMarker;
  ...
  var activate_SkewT = function activate_SkewT(latLon) {
    set_dimensions();
    var btn = document.getElementById('close-tooltips').addEventListener('click', function () {
    ...
    var showSondesCB = document.getElementById('show-sondes-checkbox').checked;
    if (sondeMarker) map.removeLayer(sondeMarker);
    sondeMarker = L.marker([lat, lon]).addTo(map);

    if (showSondesCB) {
      fetch("".concat(SkewTApiPath, "/api/nearest/?lat=").concat(lat, "&lon=").concat(lon)).then(function (response) {
        return response.json();
      }).then(function (sonde) {
        var sondeValidtime = new Date(sonde.sonde_validtime);
        var sondeHour = sondeValidtime.getHours().toString();
        ...
        M.bindTooltip("".concat(sonde.station_name, " (").concat(sondeHour, "Z)"), {
          permanent: true,
          direction: 'right',
          interactive: true,
          offset: [15, 0],
    ...
    load('airData', dataOptions).then(function (_ref2) {
      ...
      var U = Ums.map(function (datapoint) {
        return datapoint *= 1.94384;
      });
      var V = Vms.map(function (datapoint) {
        return datapoint *= 1.94384;
      });
  ...
  var onMarkerClick = function onMarkerClick(wmo_id, startpressure, endpressure) {
    d3.selectAll('path').interrupt();
    var sonde = fetchSonde(wmo_id, startpressure, endpressure);
  };
  ...
  function fetchSonde(wmo_id, startpressure, endpressure) {
    fetch("".concat(SkewTApiPath, "/api/sondes/?wmo_id=").concat(wmo_id)).then(function (response) {
      ...
      var U = sonde.u_windMS.map(function (datapoint) {
        return datapoint *= 1.94384;
      });
      var V = sonde.v_windMS.map(function (datapoint) {
        return datapoint *= 1.94384;
      });
      var sonde_timestamp = sonde.sonde_validtime;
      var dataOptions = {
        model: 'sonde'
      };
